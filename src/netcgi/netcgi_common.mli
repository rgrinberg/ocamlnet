...
(** [new in_obj_of_descr ~buffer fd] creates a
    {!Netchannels.in_obj_channel} enriched with some methods to input
    until a given character is reached.

    Remark: The method [#close_in()] does {b not} close the file
    descriptor [fd] because it usually also serves for output.  It
...
object
  ...
  val in_buf : string
    (** The internal buffer of length [buffer_len].  The data in the
        [in_buf] is at indexes [i] s.t. [in0 <= i < in1]. *)
  val mutable in0 : int
  val mutable in1 : int
    (** Invariant: [0 <= in0]; [in1 <= String.length buffer]. *)
  ...
  method private fill_in_buf : unit -> unit
    (** [#fill_in_buf()] refills [in_buf] if needed (when empty).  After
        ...
        operation would block (then [in0 = 0] also).

        @raise End_of_file if no more data is available. *)

  method input_all_till : char -> string
    (** [#input_all_till c] returns the next chunk of input until [c]
        is reached.  [c] is not returned and is skipped.

        @raise End_of_file if the end of file is reached before any
        character is read.

        @raise Sys_blocked_io for non-blocking I/O if, before [c] is
        found, we find ourselves in a situation where there are no
        bytes to read.  *)
  method private unsafe_input : string -> int -> int -> int
    (** [unsafe_input buf ofs len] same as [#input] but does not check
        whether the channel is closed whether [ofs] and [len]
        designate a valid substring of [buf]. *)
  method private unsafe_really_input : string -> int -> int -> unit
    (** [unsafe_really_input buf ofs len] same as [#really_input] but
        does not check whether the channel is closed whether [ofs] and
        [len] designate a valid substring of [buf]. *)

(*  method open_in : unit -> unit
    (** [open_in()] reopens the channel (i.e., reenable safe methods). *)
*)
end


(** [new out_obj_of_descr ~buffer fd] creates a buffered
    {!Netchannels.out_obj_channel}.

    Remark: The method [#close_out] does {b not} close the file
...
object
  ...
  method private write : string -> int -> int -> int
    (** [write buf ofs len] writes the substring [buf.[ofs
  ...
  method private unsafe_output : string -> int -> int -> int
    (** [unsafe_output buf ofs len] same as [#output] but does not
        check whether the channel is closed whether [ofs] and [len]
        designate a valid substring of [buf]. *)
  method private unsafe_really_output : string -> int -> int -> unit

(*  method open_out : unit -> unit
    (** [open_out()] reopens the channel (i.e., reenable safe methods). *)
*)
end
...
val is_prefix : string -> string -> bool
  (** [is_prefix pre s] checks whether [pre] is a prefix of [s].  This
      is a naive implementation (e.g. not using Knuth-Morris-Pratt
      ideas) but it has lower startup cost (good for small prefixes).  *)
