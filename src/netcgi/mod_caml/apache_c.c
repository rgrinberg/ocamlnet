 ...
 *
 * $Id: apache_c.c,v 1.26 2006/02/28 17:58:54 rwmj Exp $
 */
...
{
  static value *http_error = NULL;
  if (http_error == NULL) {
    http_error = caml_named_value("mod_caml_http_error");
  }
  raise_with_arg(*http_error, Val_int(i));
}
...
static int
mod_caml_table_get_loop(void *res, const char *key, const char *val)
{
  CAMLparam0();
...
CAMLprim value
mod_caml_table_get_all(value tv, value str)
{
  CAMLparam2(tv, str);
  CAMLlocal1(res); /* list */
  table *t = Table_val(tv);
  ...
  ap_table_do(&mod_caml_table_get_loop, &res, t, key, NULL);
  CAMLreturn(res);
}
...
static int
mod_caml_table_fields_loop(void *res, const char *key, const char *val)
{
  CAMLparam0();
...
CAMLprim value
mod_caml_table_fields(value tv)
{
  CAMLparam1(tv);
  CAMLlocal1(res); /* list */
  table *t = Table_val(tv);
  ...
  ap_table_do(&mod_caml_table_fields_loop, &res, t, NULL);
  CAMLreturn(res);
}
...
CAMLprim value
mod_caml_table_add (value tv, value key, value val)
{
  CAMLparam3 (tv, key, val);
  table *t = Table_val (tv);
...
CAMLprim value
mod_caml_server_admin(value sv)
{
  CAMLparam1(sv);
  server_rec *s = Server_rec_val(sv);
  if (s->server_admin)
...
CAMLprim value
mod_caml_server_is_virtual(value sv)
{
  CAMLparam1(sv);
  server_rec *s = Server_rec_val(sv);
...
CAMLprim value
mod_caml_request_status_line (value rv)
{
  CAMLparam1 (rv);
  request_rec *r = Request_rec_val (rv);
  if (r->status_line)
...
CAMLprim value
mod_caml_request_set_status_line (value rv, value str)
{
  CAMLparam2 (rv, str);
  request_rec *r = Request_rec_val (rv);
...
CAMLprim value
mod_caml_request_status (value rv)
{
  CAMLparam1 (rv);
  request_rec *r = Request_rec_val (rv);
...
CAMLprim value
mod_caml_request_set_status (value rv, value i)
{
  CAMLparam2 (rv, i);
  request_rec *r = Request_rec_val (rv);
  r->status = Int_val (i);
...
CAMLprim value
mod_caml_request_rflush (value rv)
{
  CAMLparam1 (rv);
  request_rec *r = Request_rec_val (rv);
  int i = ap_rflush (r);
...
CAMLprim value
mod_caml_request_get_client_block_buffered(value rv, value bufv,
...
CAMLprim value
mod_caml_request_output(value rv, value bufv, value ofsv, value lenv)
{
  CAMLparam4(rv, bufv, ofsv, lenv);
  request_rec *r = Request_rec_val(rv);
  int ofs = Int_val(ofsv);
...
mod_caml_get_server_config (value rv)
{
  ...
  if (r->server && r->server->module_config) {
    void *c = ap_get_module_config (r->server->module_config, &caml_module);
    if (c) config = *(value *) c;
    else goto not_found;
  } else
  not_found:
...
mod_caml_get_dir_config (value rv)
{
  ...
  request_rec *r = Request_rec_val (rv);
  if (r->per_dir_config) {
    void *c = ap_get_module_config (r->per_dir_config, &caml_module);
    if (c) config = *(value *) c;
    else goto not_found;
  } else
  not_found:
