 ...
 * {!Xdr_mstring.mstring}.
 *
 * The term [X_direct(t,read)] is generated by ocamlrpcgen at positions
 ...
 * needs to be postprocessed by ocamlrpcgen-generated code. Note that the
 * type only contains the function for reading XDR back. The function for
 * writing must be put into the corresponding [XV_direct] node of the values.
 *)
...
type xdr_value =
  ...
  | XV_direct of exn * int * (string -> int ref -> unit)
      (* The variant [XV_direct(exn,size,write)] should only be used by
	 ...
	 of bytes in XDR format. [write] is a function
	 called as [write s pos] meaning that the XDR-encoded value is 
	 written into string [s] at byte position [!pos]. The position
	 [pos] is increased by the number of written bytes.
       *) 
...
val unpack_xdr_value_l : ?pos:int -> ?len:int -> ?fast:bool -> ?prefix:bool ->
   ...
   *
   * [direct]: If true, an [XV_direct] node is generated when an [X_direct]
   * is found in the type. This enables direct mapping, and is normally only
   * meaningful in conjuction with the ocamlrpcgen-generated language mapping.
   *
...
val write_string_fixed : Rtypes.uint4 -> string -> string -> int ref -> unit
