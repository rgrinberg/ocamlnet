...
(* --- PLANNED
   Currently implemented in Pxp_reader, but better we do it here.
...
val create_unicode_lexbuf : 
      ?raw_size:int -> ?char_size:int -> encoding -> unicode_lexbuf
  (** Creates an empty [unicode_lexbuf] to analyse strings of the 
   * passed [encoding].
   *
...
val unicode_lexbuf_from_string :
...
val unicode_lexbuf_from_string_inplace :
      encoding -> string -> unicode_lexbuf
  (** Creates a [unicode_lexbuf] analysing the passed string encoded in
...
val delete_from_unicode_lexbuf :
...
val refill_unicode_lexbuf :
   ...
   *   return value 0 means EOF.
   *)

val close_unicode_lexbuf :
      unicode_lexbuf -> unit
 ...
 * 
 * $Log$
 * Revision 2.13  2004/07/11 15:41:18  stolpmann
 * 	Added: IBM-1047 charset
 *
 * Revision 2.12  2004/07/10 23:43:06  stolpmann
 * 	ocamldoc
 *
 * Revision 2.11  2004/07/08 22:10:19  stolpmann
 * 	Adding accelerators for character set conversion.
 *
 * Revision 2.10  2004/07/07 22:41:54  stolpmann
 * 	Added Netulex, Netaux.ArrayAux.
 * 	Some improvements in Netconversion.
 *
 * Revision 2.9  2004/07/04 19:19:02  stolpmann
 * 	Updates ocamldoc
 *
 * Revision 2.8  2004/06/09 22:51:57  stolpmann
 * 	Enhanced cursors: Support for UTF-16 with BOMs, support for
 * incomplete multi-byte characters, blitting functions.
