 ...
 *
 * $Log$
 * Revision 2.16  2004/11/24 18:01:14  gremlin43820
 ...
 *
 * Revision 2.15  2004/07/06 15:20:53  stolpmann
 * 	Bugfixes: write_value now correctly folds long encoded words.
 *
 * Revision 2.14  2004/07/04 18:51:54  stolpmann
 * 	Added: split_mime_type, split_uri
 *
 * Revision 2.13  2004/05/30 21:22:00  stolpmann
 * 	Fixed bug when parsing backslashes within double quotes
 *
 * Revision 2.12  2002/02/02 16:10:47  stolpmann
 * 	Bugfix: read_header recognizes empty headers.
 *
 * Revision 2.11  2002/02/01 23:26:22  stolpmann
 * 	Fixed some problems with mail from the real world
 *
 * Revision 2.10  2002/01/23 22:15:18  stolpmann
 * 	New function: [create_boundary]
 *
 * Revision 2.9  2002/01/21 00:45:39  stolpmann
 * 	[scan_header]: New option ~strip, so you can turn off unfolding
 * but leave whitespace stripping on.
 * 	New: [write_header], [mk_param], [write_value], [param_tokens]
 *
 * Revision 2.8  2002/01/12 18:07:34  stolpmann
 * 	Added support for RFC 2231: get_language,
 * scan_value_with_parameters_ep, and scan_mime_type_ep. There is one
 * incompatible change regarding EncodedWord. The definition has been
 * changed from EncodedWord(charset,encoding,value) to
 * EncodedWord((charset,language),encoding,value), i.e. the first
 * component is now the pair of the character set and the language.
 * EncodedWord is still a triple, so code ignoring the first component
 * will continue working (e.g. match...with EncodedWord(_,_,_) -> ...)
 *
 * Revision 2.7  2002/01/06 02:22:09  stolpmann
 * 	New parser for headers: [read_header].
 * 	New parser for multipart messages: [read_multipart_body]. The
 * old functions [scan_multipart_body], [scan_multipart_body_and_decode],
 * and [scan_multipart_body_from_netstream] are derived from this new
 * parser.
 *
 * Revision 2.6  2002/01/05 19:40:31  stolpmann
 * 	Follow-up change: method [delete] of netstreams renamed into
 * [skip].
 *
 * Revision 2.5  2002/01/04 22:01:06  stolpmann
 * 	No longer using deprecated Base64 functions.
 *
 * Revision 2.4  2001/12/28 21:14:00  stolpmann
 * 	[scan_multipart_body_from_netstream] uses now the new
 * object-oriented interface of Netstream.
 * 	Renamed Str into S to make clear that this module is not
 * the standard Str module.
 *
 * Revision 2.3  2001/10/17 04:32:52  pdoane
 * Updated for 3.03
 *
