 ...
 * 	ocamldoc
 *
 * Revision 1.7  2003/01/27 23:30:00  stolpmann
 * ^	New property "ocamlnet.https"
 *
 * Revision 1.6  2002/04/12 21:48:56  stolpmann
 * 	Better support for mod_jk.
 *
 * Revision 1.5  2002/03/24 22:30:05  stolpmann
 * 	Splitted the function [server] up into the parts [server_init]
 * and [server_loop]. This is useful to implement the `Process_pool
 * execution model: [server_init] can be called by the master process,
 * and [server_loop] can run in every subprocess.
 * 	New: These functions have the argument ?controlpipe that specifies
 * how to handle the control pipe.
 * 	New: [read_control_pipe]
 * 	Change: The option ?select of the server functions has been
 * extended to ?select_accept.
 *
 * Revision 1.4  2002/02/04 15:21:31  stolpmann
 * 	Updated comments.
 *
 * Revision 1.3  2001/12/09 20:25:24  stolpmann
 * 	Added mt options for prng_init.
 *
 * Revision 1.2  2001/12/09 02:42:23  stolpmann
